...
combine :: proc(date, time: Date) -> Date {
    date := date
    time := time 
    if (date.month == 0 && time.month != 0) || 
       (f32(date.hour + date.minute) + date.second != 0 && f32(time.hour + time.minute) + time.second == 0) {
        date, time = time, date
    } 
    ...
    datetime.offset_hour   = time.offset_hour  
    datetime.offset_minute = time.offset_minute

    datetime.is_time_only  = false
    datetime.is_date_only  = false
    datetime.is_date_local = time.is_date_local
    return datetime
}
