...
@private find_slice :: proc(A: [] $T, b: T) -> int { for a, i in A { if a == b do return i } return len(A) }
...
@private
get_indent :: proc(size: int) -> string {
    size := min(size, len(INDENTATION))
